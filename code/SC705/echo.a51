...
;
; Simply echoes back any character received. Serial post is set
; for 115200,8,N,1 assuming that a 7.3728MHz crystal was fitted to X1.
;
...
LOOP2:
	...
	INC		DPTR
	JZ		ECHO			; start echoing if end of string to transmit
...
WAIT1:
	...
	; echo back any characters recevied
ECHO:
	MOV		DPTR,#CTRL
ECHO1:
	MOVX	A,@DPTR			; read status register
	JNB		ACC.0,ECHO1		; wait for rx reg to be full

	; got a character so read it in
	INC		DPTR			; DPTR points to the data register
	MOVX	A,@DPTR			; read the character
	MOV		R0,A			; and save it

	MOV		DPTR,#CTRL
ECHO2:
	MOVX	A,@DPTR			; read status register
	JNB		ACC.1,ECHO2		; wait for tx reg to be empty

	MOV		A,R0			; get the character back
	INC		DPTR			; DPTR points to the data register
	MOVX	@DPTR,A			; write character to send
	SJMP	ECHO			; got back and wait for another character
...
TITLE:
	DB		'SC705 (MC68B50) Echo Demo',10,13,0
MSG:
	DB		'Any keys pressed are echoed back.',10,13,0
